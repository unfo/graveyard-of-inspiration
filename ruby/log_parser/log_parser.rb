...
# representations.
# 
# Extended by Wolfgang Wopperer so that the log file is parsed reversely and only for a defined time interval.
#
# Authors:: Jan Wikholm [jw@jw.fi], Wolfgang Wopperer [wolfgang.wopperer@mindmatters.de]
# License:: MIT

require 'logger'
require File.join(File.dirname(__FILE__), "../lib/io_ext.rb")
...
class LogParser
  ...
  # 
  # NEW: Reversely parsing only the latest entries (for large files)
  # as defined in the interval parameter (in seconds)
  def parse_io_stream(stream, interval)
    stats = []
    lines_parsed = 0
    begin
      stream.reverse_each do |line|
        lines_parsed += 1
        ...
        stats = generate_stats(raw_data)
        %r{.*:([^/]+:[^/]+:[^/]+)\s.*}.match(stats['DATETIME'])
        break if Time.parse($1) < Time.now - interval
        yield stats
      end
    end
  end
